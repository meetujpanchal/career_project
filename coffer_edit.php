<?php
 ...
 if(isset($_POST['btnsub'])){
    $result=mysqli_query($cn,"Update facility_offer set c_id='".$_POST['clgid'].", clgname='".$_POST['clgname'].",cname='".$_POST['cname']."'where f_id=".$_REQUEST['foffer_id'])or die("Error In Update Query");
    header("location:course_offer.php");
   exit();
 }
 $result=mysqli_query($cn,"select * from cource_offer where oid=".$_REQUEST['coffer_id'])or die("Error in select Query");
...
<body>
  ...
  <option value="<?php echo $cr['c_id']?>"><?php echo $cr['clgname'];?></option>
  <?php } ?>
   </select>
                                </div>
                            </div>
  ...
  ?>
   <select name="cname" class="input--style-4">
   <?php
  ...
  <?php } ?>
    ...
    $catresult=mysqli_query($cn,"Select * from college")or die("Error In Select Query");
  ?>
   <select name="clgname" class="input--style-4">
   <?php
    while($cr=mysqli_fetch_array($catresult)){ ?>
